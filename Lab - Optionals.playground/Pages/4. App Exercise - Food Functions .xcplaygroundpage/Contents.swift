...
func check(_ mealName:String)->Meal?{
    return meals[mealName]
}

if let meal = check("Breakfast") {
    print("Meal found:) \(meal.food)")
} else {
 ...
 */
func mealCheck(_ meallog:String)->[String: Any]?{
    return UserDefaults.standard.dictionary(forKey: meallog)
}


if let meal1 = mealCheck("Lunch"){
    print("Meal Found \(meal1)")
}else{
    print("Meal not Found")
}
